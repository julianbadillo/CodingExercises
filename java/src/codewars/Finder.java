...
public class Finder {
    ...
    static List<String> cheapestPath(int[][] t, Point start, Point finish) {
    	...
    	int max = Arrays.stream(t).flatMapToInt(row -> Arrays.stream(row)).sum();
    	for(int[] row: cost) Arrays.fill(row, max);
    	
    	Dir[][] dir = new Dir[r][c];
    	
    	// do a BFS on the map
    	PriorityQueue<Point> queue = new PriorityQueue<>((Point p1, Point p2) -> cost[p1.x][p1.y] - cost[p2.x][p2.y]);
